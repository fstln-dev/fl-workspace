...
description: |
  ...
  一键初始化 OpenWork Native 工作空间，支持飞书集成配置。
---
# 工作空间初始化

快速初始化 OpenWork Native 工作空间，创建标准目录结构和配置文件。
...
```bash
# 如果 .opencode-workspace 已存在，检查版本
...
│  feishu (第三方 MCP)                                         │
│  → Wiki、多维表格、任务、日历、消息等                         │
│  → 需要在 OpenWork Extension 中手动配置                      │
│  → 配置后需要完成浏览器授权                                  │
└─────────────────────────────────────────────────────────────┘

⚠️ 注意：OpenWork 的 MCP 配置需要在 Extension 中手动添加
...
获取 URL 后，请在 OpenWork Extension 中添加：
...
✅ 创建 OPENWORK.md
✅ 创建 .opencode/settings.json
✅ 创建 .opencode/context/team.md
✅ 创建 .opencode/context/doc-index.md
✅ 创建 .opencode/context/feishu-config.md
...
```
project-root/
├── OPENWORK.md                 # AI 配置（自动加载）
├── .opencode/
│   ├── settings.json            # OpenWork 设置
│   └── context/
│       ├── team.md              # 团队成员
│       ├── doc-index.md         # 文档索引
...
1. 编辑 .opencode/context/team.md 添加团队成员

2. 完成飞书 MCP 配置（如果尚未配置）:
   在 OpenWork Extension 中添加 MCP 服务器：
   - feishu-official: 从 https://open.feishu.cn/page/mcp 获取 URL
   - feishu: https://feishu-mcp.fastgrowth.ai/mcp
...
3. 点击「重新授权」
4. 复制新的服务器 URL
5. 在 OpenWork Extension 中更新 URL
6. 重启 OpenWork
